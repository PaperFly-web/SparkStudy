...
object Spark01_WordCount_Top10_1 {
  def main(args: Array[String]): Unit = {
    ...
    val clickRDD: RDD[(String, (Int, Int, Int))] = actionRDD.filter(line => {
    ...
    val orderRDD: RDD[(String, (Int, Int, Int))] = actionRDD.filter(line => {
      val words: Array[String] = line.split("_")
      words(8) != "null"
    }).flatMap(line => {
      val words: Array[String] = line.split("_")
    ...
    val payRDD: RDD[(String, (Int, Int, Int))] = actionRDD.filter(line => {
      val words: Array[String] = line.split("_")
      words(10) != "null"
    }).flatMap(line => {
      val words: Array[String] = line.split("_")
    ...
    val analysisRDD: RDD[(String, (Int, Int, Int))] = clickRDD.union(orderRDD).union(payRDD).reduceByKey((t1, t2) => {
